...
describe('Sprint 7', () => {
    ...
    describe('Profile functions tests', () => {
        ...
        describe('empty-cards', () => {
            it('empty-booking', () => {
                getContent();
                cy.intercept('GET', /users\/me/, USER_NO_BOOKING).as('me');
                cy.visit('http://localhost:3000/#/profile');
                cy.wait(['@books', '@me', '@categories']);
                cy.wait(5000);
                ...
                cy.get('[data-test-id=empty-blue-card]')
                    .should('have.css', 'background-color', 'rgb(110, 118, 241)')
                    .should('have.css', 'opacity', '0.7');
            });
            it('empty-handed', () => {
                getContent();
                cy.intercept('GET', /users\/me/, USER_NO_DELIVERY).as('me');
                cy.visit('http://localhost:3000/#/profile');
                cy.wait(['@books', '@me', '@categories']);
                cy.wait(5000);
                cy.get('[data-test-id=empty-blue-card]')
            ...
            it('empty-history', () => {
                getContent();
                cy.intercept('GET', /users\/me/, USER_NO_HISTORY).as('me');
                cy.visit('http://localhost:3000/#/profile');
                cy.wait(['@books', '@me', '@categories']);
                cy.wait(5000);
                cy.get('[data-test-id=empty-blue-card]')
        ...
        describe('expired', () => {
            it('expired-booking', () => {
                getContent();
                cy.intercept('GET', /users\/me/, USER_EXPIRED_BOOKING).as('me');
                cy.visit('http://localhost:3000/#/profile');
                cy.wait(['@books', '@me', '@categories']);
            ...
            it('expired-handed', () => {
                getContent();
                cy.intercept('GET', /users\/me/, USER_EXPIRED_HANDED).as('me');
                cy.visit('http://localhost:3000/#/profile');
                cy.wait(['@books', '@me', '@categories']);
                cy.get('[data-test-id=expired]')
                    .should('be.visible')
        ...
        const updateReviewOnBookPage = (number, book) => {
            ...
            if (book.id === 34) {
                cy.get('[data-test-id=button-rate-book]')
                    .should('be.visible')
                    .should('be.enabled')
                    // .should('have.css', 'background', 'rgba(0, 0, 0, 0) linear-gradient(231.58deg, rgb(248, 54, 0) -53.35%, rgb(249, 212, 35) 297.76%) repeat scroll 0% 0% / auto padding-box border-box')
                    // такой вариант проверки предлагает сам cypress - просто уберу проверку цвета
                    .should('have.css', 'border-radius', '30px')
